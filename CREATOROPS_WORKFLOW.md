...
여러 브랜드를 한 워크스페이스에서 관리하면서, 각 브랜드가 원하는 조건에 맞춰 AI가 인플루언서 후보를 찾고, 추천 이유와 페르소나를 기록하며, 자동/수동 섭외를 분류하고, 콘텐츠 업로드 후 성과 데이터를 추적해 보고서로 다운로드할 수 있는 시스템을 만든다.

## 주요 사용자 흐름

### 0. 브랜드 워크스페이스 선택

운영자는 좌측 Brand Workspace에서 관리할 브랜드를 선택하거나 새 브랜드를 추가한다.
...
제안/응답 발송은 두 종류로 분리된다.

- 자동 섭외: AI 추천 결과에서 `메시지 검토함`을 누르면 생성
...
승인 대기 -> 발송 완료 -> 응답 -> 섭외 완료
          \-> 캠페인 제외 -> 현재 캠페인에서만 발송 차단
          \-> 전체 수신 거부 -> 모든 캠페인 이메일 발송 차단
```

일반적인 거절이나 이번 캠페인 미참여는 `캠페인 제외`로 처리한다. 수신자가 앞으로의 이메일을 보내지 말라고 명시한 경우에만 `전체 수신 거부`로 처리한다. 운영자가 잘못 등록한 경우 상세 화면에서 각 범위를 해제하며 등록·해제 이력은 감사 로그에 남긴다.
