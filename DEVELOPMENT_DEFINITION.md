...
| 메시지 검토함 | 구현 완료 | 상세 보기, 복사, 연락 채널 열기, 응답 처리 |
| 섭외 완료 풀 | 구현 완료 | 캠페인별 저장 및 광고주 컨펌용 지표 표시 |
...
- 상세 보기 모달에서는 크리에이터, 캠페인, 상태, 연락 방식, 추천/발송 근거, 제안 메시지 전문을 확인한다.
...
- 섭외 완료 풀은 캠페인별로 귀속된다.
- 광고주 컨펌에 필요한 팔로워, 평균 조회수, 참여율, 예상 단가, 브랜드 적합도, 데이터 품질이 표시된다.
