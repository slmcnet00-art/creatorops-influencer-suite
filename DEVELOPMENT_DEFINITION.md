...
| 발굴 | 후보 검색과 추천 | 브랜드 조건 설정, 브리프 자동 세팅, 실제 웹 발굴, AI 추천, 수동 발굴, 발굴 서치 풀/메시지 전 후보 풀 전환 |
...
추가 보완:
- 발굴 화면 안에서 `발굴 서치 풀`과 `메시지 전 후보 풀`을 전환해 확인한다.
- `발굴 서치 풀`은 검색/필터/실제 웹 발굴 결과를 보여주고, `메시지 전 후보 풀`은 쇼트리스트에 저장됐지만 아직 메시지 검토함으로 보내지 않은 후보만 보여준다.
- `메시지 전 후보 풀`은 엑셀 다운로드를 지원한다.
...
- 메시지 상태 보드의 전체/검토함/발송완료/응답/캠페인 제외/전체 수신 거부 카드를 누르면 해당 상태만 필터링해서 볼 수 있다.
...
- 앱을 열거나 새로고침했을 때 하루 1회 자동 갱신을 실행하고, 리포트의 새로고침 버튼을 누르면 즉시 갱신한다.
...
- 필터는 국가 프리셋(전체, KR, US, JP, CN, SEA, EU), 미디어 유형, 플랫폼, 조회수 순, 팔로워 대비 터진 콘텐츠 순, 공유 수, 최신순을 지원한다.
...
| POST `/outreach/suppressions` | 구현 | 캠페인 제외와 워크스페이스 전체 수신 거부를 구분해 등록·해제하고 감사 로그 저장 |
...
- `TIKTOK_COMMERCIAL_DATE_WINDOW_DAYS`
- `OPENAI_API_KEY`
- `OPENAI_MODEL`
- `GMAIL_CLIENT_ID`
- `GMAIL_CLIENT_SECRET`
...
내부 운영자/데이터팀/개발팀/CS팀이 대시보드 지표의 원천 데이터를 추적할 수 있도록 `데이터룸` 메뉴를 추가했다.

- 접근 권한: Owner/Admin/Operator 계정과 Analyst 계정에서 접근 가능, Client 계정은 비노출
- 요약 영역: 전체 raw 데이터 수, 정상/지연/오류/중단 상태, 내부/외부 데이터 수, 계산지표 수, 마지막 동기화 시간
- Raw 데이터 관리: 내부/외부 탭, 상태/카테고리/수집방식/담당부서/검색 필터, 원천 위치, 저장 위치, 연결 계산지표, 품질 이슈, 로그 위치 표시
- 계산지표 관리: 내부/외부 탭, 지표 번들별 그룹, 계산식, 사용 raw 데이터 ID/이름, 상태, 기준 기간, 이상치 기준, 신뢰도 표시
- 상세 패널: raw 데이터 또는 계산지표 클릭 시 우측 패널에서 원천/저장 위치, 최근 로그, 연결 지표/데이터, 오류 이력, 담당자, 운영 메모 확인
- 운영 액션: 수집 로그 보기, 수동 재수집 요청, 계산 로그 보기, 재계산 요청 버튼을 배치했으며 현재는 토스트 안내 기반 MVP

후속 개발:

1. Supabase 테이블 또는 별도 데이터 카탈로그 테이블로 raw/metric 정의 저장
2. Render API 로그, 수집 job 로그, 계산 job 로그와 상세 패널 연결
3. 데이터 품질 이슈 등록/담당자 지정/변경 이력 저장
4. 재수집/재계산 버튼을 실제 job queue와 연결
5. 권한별 데이터룸 필드 마스킹과 감사 로그 적용
