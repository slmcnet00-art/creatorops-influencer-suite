...
function buildDataRoomExtendedRawCatalog({
  ...
  return append([
    ...
    },
    {
      ...
      scope: '외부',
      category: '브랜드 모니터링',
      description: '외부 모니터링 API로 수집하는 브랜드/경쟁사 기준 인플루언서 리스트, 채널 링크, 구독자, 지역, 언어, 평균 조회수, 총 조회수, 참여율, 영상 수, 최근 업로드일, 예상 단가',
    ...
    },
    {
      ...
      scope: '외부',
      category: '콘텐츠 모니터링',
      description: '외부 모니터링 API로 수집하는 프로젝트 요약, 콘텐츠별 상세 성과, 일별 변화 데이터',
    ...
    },
    {
      ...
      scope: '외부',
      category: '성과 변화/기여도',
      description: 'Video Monitor Workbench에서 내려받은 델타 랭킹, 조회/좋아요/댓글 추이, 인플루언서 기여도, 라벨 기여/분포 데이터',
      purpose: '어떤 콘텐츠와 인플루언서가 캠페인 성장을 만들었는지 델타와 기여율 기준으로 분석',
      method: '수동 업로드 / 외부 리포트',
      cycle: '리포트 업로드 시 / 보고서 작성 전',
      lastCollectedAt: '-',
    ...
    },
    {
      ...
      status: creatorGroups.length ? '정상' : '미수집',
...
function buildDataRoomExtendedMetricCatalog({
  ...
  rows.push(
    ...
    ['MET-EXT-VIDEO-002', '외부 모니터 콘텐츠 참여율', '외부 API/벤치마크 번들', '외부', '콘텐츠별 좋아요, 댓글, 공유, 저장을 조회수로 나눈 반응률', '(likes + comments + shares + collects) / views', ['RAW-EXT-MON-VIDEO-001'], '콘텐츠별/일별', 'API 수집 시', '검증 필요', '리포트, 콘텐츠 성과', '인플루언서별 콘텐츠 품질과 후킹 반응 비교', 'ER 0 또는 20% 이상', '중간', '데이터팀', 'report_metric_snapshots', 'Instagram/TikTok 저장/공유 공개 여부에 따라 신뢰도 표시'],
    ['MET-EXT-VIDEO-003', '외부 모니터 일별 성장 추이', '외부 API/벤치마크 번들', '외부', 'daily-change API 응답의 일별 조회수/좋아요/댓글 변화', 'daily_delta(metric_t) = metric_t - metric_t-1', ['RAW-EXT-MON-VIDEO-001'], '일별', 'API 수집 시', '검증 필요', '리포트, 브랜드 인사이트 추이 차트', '어떤 날짜에 콘텐츠가 터졌는지 성장 구간 식별', '음수 델타 또는 누락 날짜', '중간', '데이터팀', 'daily_change_import_logs', 'daily-change API 응답 행 구조와 누락 날짜 확인'],
