...
function App() {
  ...
  const kpiReadiness = useMemo(() => {
    const totalCampaigns = Math.max(brandCampaigns.length, 1)
    const structuredCampaigns = campaignKpiSummaries.filter((summary) => summary.metrics.length > 0).length
    const averageProgress = campaignKpiSummaries.length
      ? Math.round(campaignKpiSummaries.reduce((sum, summary) => sum + summary.progress, 0) / campaignKpiSummaries.length)
      : 0

    return {
      structuredCampaigns,
      averageProgress,
      coverage: Math.round((structuredCampaigns / totalCampaigns) * 100),
    }
  }, [brandCampaigns.length, campaignKpiSummaries])
  ...
  const pmExecutionItems = useMemo(
    () => {
      const realCreatorCount = creators.filter((creator) => !isExampleCreator(creator)).length
      const apiReadyCount = dataCoverage.officialReady
      const outreachReady = activeOutreach.length
      const reportReady = activeTrackedPosts.length
      const poolReady = activeRecruitedPool.length
      const hasLiveDiscovery = realCreatorCount > 0
      const hasTeamBackend = false
      const hasAutomatedEmail = false
      const hasStructuredKpi = kpiReadiness.structuredCampaigns === Math.max(brandCampaigns.length, 1)

      return [
        {
          area: '실제 후보 발굴',
          status: hasLiveDiscovery ? '진행 가능' : 'API 연결 필요',
          progress: hasLiveDiscovery ? 72 : 28,
          owner: 'Growth / Dev',
          next: hasLiveDiscovery
            ? '후보 수치 검증과 쇼트리스트 운영'
            : 'YouTube Data API 또는 Google Search API/CX 연결',
          evidence: `${realCreatorCount}명 실제 후보 · 공식 API 대상 ${apiReadyCount}명`,
          tone: hasLiveDiscovery ? 'ready' : 'blocked',
        },
        {
          area: 'AI 추천/메시지',
          status: activeRecommendations.length ? '검토 가능' : '추천 실행 필요',
          progress: activeRecommendations.length ? 66 : 40,
          owner: 'PM / Marketer',
          next: activeRecommendations.length
            ? '상위 후보 메시지 검토함 저장'
            : '실제 후보 확보 후 AI 매칭 실행',
          evidence: `${activeRecommendations.length}명 추천 · 메시지 ${outreachReady}건`,
          tone: activeRecommendations.length ? 'ready' : 'watch',
        },
        {
          area: '팀 공유 DB',
          status: hasTeamBackend ? '운영 가능' : '구축 필요',
          progress: hasTeamBackend ? 80 : 20,
          owner: 'Backend',
          next: 'Supabase/Auth/역할 권한/감사 로그 연결',
          evidence: '현재 localStorage 로컬 전용',
          tone: 'blocked',
        },
        {
          area: '발송/응답 운영',
          status: hasAutomatedEmail ? '자동 발송 가능' : '수동 검토 중심',
          progress: hasAutomatedEmail ? 78 : 45,
          owner: 'Ops',
          next: 'Gmail API와 수신거부/중복발송 로그 연결',
          evidence: `승인 대기 ${activeOutreach.filter((item) => item.status === '승인 대기').length}건 · 응답 ${activeOutreach.filter((item) => item.status === '응답').length}건`,
          tone: hasAutomatedEmail ? 'ready' : 'watch',
        },
        {
          area: '성과/리포트',
          status: reportReady && hasStructuredKpi ? '목표 대비 리포트 가능' : 'KPI/추적 보강 필요',
          progress: Math.max(reportReady ? 70 : 35, hasStructuredKpi ? 68 : 42),
          owner: 'PM / Analyst',
          next: reportReady
            ? '목표 대비 미달 캠페인 재섭외/후속 콘텐츠 판단'
            : '콘텐츠 URL과 조회/댓글/공유/전환 수집',
          evidence: `${reportReady}건 콘텐츠 · KPI 구조화 ${kpiReadiness.structuredCampaigns}/${brandCampaigns.length}개 · 섭외 풀 ${poolReady}명`,
          tone: reportReady && hasStructuredKpi ? 'ready' : 'watch',
        },
      ]
    },
    [
      ...
      creators,
      dataCoverage.officialReady,
      kpiReadiness.structuredCampaigns,
    ],
  )

  const pmReadinessScore = useMemo(
    () =>
      Math.round(
        pmExecutionItems.reduce((sum, item) => sum + item.progress, 0) /
          Math.max(pmExecutionItems.length, 1),
      ),
    [pmExecutionItems],
  )

  const pmCriticalItem = pmExecutionItems.find((item) => item.tone === 'blocked') ?? pmExecutionItems[0]

  const pmPriorityItems = useMemo(
    () => [
      {
        priority: 'P0',
        title: '팀 공유 DB / 권한 전환',
        reason: '한 명의 브라우저 저장소로는 팀 운영, 클라이언트 컨펌, 개인정보 접근 제어가 막힙니다.',
        action: 'Supabase 또는 백엔드 DB, Auth, 역할 권한, 감사 로그를 다음 개발축으로 고정',
        status: '개발 필요',
        tone: 'blocked',
      },
      {
        priority: 'P0',
        title: 'KPI 구조화',
        reason: '조회수, 전환, 주문, 매출 목표가 숫자로 분리되어야 캠페인 성공/미달 판단이 됩니다.',
        action: `이번 반영: 캠페인별 목표 수치와 현재 달성률 표시 · 구조화 ${kpiReadiness.coverage}%`,
        status: '1차 반영',
        tone: 'ready',
      },
      {
        priority: 'P1',
        title: '실제 후보 데이터 수집',
        reason: '예시 후보가 아니라 공개 API/검색에서 수집한 후보가 추천과 섭외의 출발점입니다.',
        action: `${dataCoverage.officialReady}명 공식 API 대상 · YouTube/Google Search 키 연결 후 후보 DB 축적`,
        status: dataCoverage.officialReady ? '진행 중' : 'API 연결 필요',
        tone: dataCoverage.officialReady ? 'watch' : 'blocked',
      },
      {
        priority: 'P1',
        title: '발송/응답 운영 로그',
        reason: '대량 섭외는 중복 발송 방지, 수신 거부, 응답률 기록이 쌓여야 다음 추천 모델이 좋아집니다.',
        action: `현재 메시지 ${activeOutreach.length}건 · Gmail/Outlook API 연결 전까지 수동 검토 중심`,
        status: '수동 운영',
        tone: 'watch',
      },
    ],
    [activeOutreach.length, dataCoverage.officialReady, kpiReadiness.coverage],
  )
      ...
      <main className="workspace">
        ...
        {activeSection === 'dashboard' && (
          <>
            ...
            <section className="panel pm-priority-panel" aria-label="PM 우선순위">
              <div className="panel-heading">
                <div>
                  <span className="mini-label">PM Priority Map</span>
                  <h2>수정 우선순위</h2>
                </div>
                <div className="pm-readiness-score">
                  <span>KPI 구조화</span>
                  <strong>{kpiReadiness.coverage}%</strong>
                </div>
              </div>
              <div className="pm-priority-list">
                {pmPriorityItems.map((item) => (
                  <article className={`pm-priority-item ${item.tone}`} key={item.title}>
            ...
            <section className="panel pm-execution-panel" aria-label="PM 실행 보드">
              <div className="panel-heading">
                <div>
                  <span className="mini-label">PM Execution Board</span>
                  <h2>운영 전환 준비도</h2>
                </div>
                <div className="pm-readiness-score">
                  <span>Readiness</span>
                  <strong>{pmReadinessScore}%</strong>
                </div>
              </div>
              <div className="pm-critical-path">
                <ClipboardList size={20} />
                <div>
                  <span>현재 핵심 병목</span>
                  <strong>{pmCriticalItem.area} · {pmCriticalItem.next}</strong>
                </div>
              </div>
              <div className="pm-execution-list">
                {pmExecutionItems.map((item) => (
                  <article className={`pm-execution-item ${item.tone}`} key={item.area}>
                    ...
                    <div className="pm-progress-bar" aria-label={`${item.area} 준비도 ${item.progress}%`}>
